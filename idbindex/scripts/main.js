...
// crea una instancia de un objeto db para que almacenemos los datos 
var db;
...
window.onload = function() {
  // En la siguiente línea, debe incluir los prefijos de las implementaciones que se desea probar.
  window.indexedDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB;

  window.IDBTransaction = window.IDBTransaction || window.webkitIDBTransaction || window.msIDBTransaction;
  window.IDBKeyRange = window.IDBKeyRange || window.webkitIDBKeyRange || window.msIDBKeyRange;
  // (Mozilla nunca ha prefijado estos objetos, por lo que no necesitamos window.mozIDB*)
  ...
  DBOpenRequest.onupgradeneeded = function(event) { 
    ...
    db.onerror = function(event) {
      console.log('Error cargando la BD.');
    };
  ...
  function displayDataByIndex(activeIndex) {
    ...
    myIndex.openCursor().onsuccess = function(event) {
      ...
      if(cursor) {
        ...
        console.log('Todas las entradas visualizadas.');    
      }
    };
  };
